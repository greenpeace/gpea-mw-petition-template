export function validation(values, formContent) {
  ...
  if (values.MobilePhone) {
    ...
    if(phone_Length == 11){
        if(pre_fix == '010'){
            phoneReg = new RegExp(
                    /^010[2-9]{1}[0-9]{3}[0-9]{4}$/
                ).test(values.MobilePhone);
        }else if(pre_fix == '011'){
            phoneReg = new RegExp(
                /^011[1|9]{1}[0-9]{3}[0-9]{4}$/
            ).test(values.MobilePhone);
        ...
        if(pre_fix == '011' || pre_fix == '016' || pre_fix == '017' || pre_fix == '018' || pre_fix == '019'){
            phoneReg = new RegExp(
    ...
    const phoneReg6 = new RegExp(/^01([0|1|6|7|8|9])?([0-9]{3,4})?([0-9]{4})$/).test(values.MobilePhone);
