      ...
      {currentQuiz?.options?.map((d, i) => {
        ...
        return (
          <Box
            ...
            border="0"
            borderRadius={'4px'}
            bgColor={checked ? ( currentQuiz?.correct ? ( d.value === currentQuiz.correct ? "#66cc00" : ( selected ? "#ba422f" : "#292F47" ) ) : ( selected ? "#292F47" : "#b7b7b7") ) : "#292F47"}
            color={'white'} 
            cursor="pointer"
          ...
          >
            <Center h="100%" position='relative'>
              ...
              {Boolean(checked && icon !== null) &&
                ( <Image src={icon} w={6} h={6} position='absolute' right={0} top='50%' transform={'translateY(-50%)'} />)
              }
            </Center>
          </Box>
        );
